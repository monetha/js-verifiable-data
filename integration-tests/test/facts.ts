...
import {
  PassportGenerator, PassportOwnership, PassportReader, FactWriter, FactReader, FactHistoryReader, FactRemover, Permissions,
...
describe('Passport creation and facts', () => {
  ...
  it('Should be able read facts history.', async () => {
    ...
    expect(cr.getAddress(response[1].factProviderAddress).basicHex).to.equal(cr.getAddress(dataSource).basicHex);
    ...
    expect(cr.getAddress(response[0].factProviderAddress).basicHex).to.equal(cr.getAddress(dataSource).basicHex);
    expect(cr.getAddress(response[1].factProviderAddress).basicHex).to.equal(cr.getAddress(dataSource).basicHex);
  ...
  it('Should be able to whitelist fact provider.', async () => {
    ...
    const method = await permissions.addFactProviderToWhitelist(factProviderAddress, passportOwner);
    const receipt = await txExecutor(method);
...
async function deleteAndValidateFact(deleteFact: (remover: FactRemover) => any) {
  ...
  const method = await deleteFact(remover);
  const transaction = await txExecutor(method);

  expect(cr.getAddress(transaction.from).basicHex).to.equal(cr.getAddress(factProviderAddress).basicHex);
